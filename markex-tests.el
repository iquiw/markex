...
(ert-deftest markex-change-region-boundary--test ()
  (with-temp-buffer
    (transient-mark-mode 1)
    (insert "foo bar baz!")
    (set-mark 8)
...
(ert-deftest markex-yank-swap--test ()
  (with-temp-buffer
    (transient-mark-mode 1)
    (insert " foo ")
    (set-mark 5)
    ...
    (should (eq (point) 5))
    (should (eq (region-beginning) 2))
    (should (eq (region-end) 5))
...
(ert-deftest markex-reset--test-with-repeat-mode ()
  (with-temp-buffer
    (transient-mark-mode 1)
    (repeat-mode 1)
    (insert "foo bar baz")
